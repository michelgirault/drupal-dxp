...
(function ($, Drupal, window, document) {
        ...
        if (typeof drupalSettings !== 'undefined' && typeof drupalSettings.theme_settings !== 'undefined' && typeof drupalSettings.theme_settings.language_dropdown !== 'undefined') {
          ...
          if (drupalSettings.theme_settings.language_dropdown === true ||  drupalSettings.theme_settings.language_dropdown === 1) {
            ...
            langNav.each(function() {
              self.languageDropdown($(this), true);
            });
   ...
   *
   */
  self.languageDropdown = function(nav, abbr) {

    var list = nav.find('ul'),
      items = list.find('li'),
      links = items.find('a'),
      activeLangeHolder = nav.find('.nav__active--language'),
      activeLink = nav.find('a.is-active'),
      activeLang = activeLink.text();

    // add class to style as a dropdown
    activeLangeHolder.addClass('nav__active--language--dropdown');
    nav.addClass('nav--language--dropdown');
    list.addClass('nav__items--language--dropdown');
    items.addClass('nav__item--language--dropdown');
    links.addClass('nav__link--language--dropdown');

    // fill in active language
    activeLangeHolder.text(activeLang);
    ...
    // activate menu (for display purposes)
